import { ArchiveRestore, Database, Download, FolderOpen, LoaderCircle, Moon, MoonStar, Monitor, Pencil, Power, RefreshCw, Save, Sun, Upload } from "lucide-react";
...
export function SettingsAdvanced({ form, onPatch, paths, backupsEpoch, onOpenPath, onRefresh }: SettingsAdvancedProps) {
  ...
  return <div className="apple-group mt-[var(--gap-section)] p-[var(--gap-card)]"><div className="flex items-center gap-4"><span className="settings-icon-tile grid h-9 w-9 shrink-0 place-items-center rounded-xl text-accent"><Database className="h-[18px] w-[18px]" strokeWidth={2} /></span><div><div className="setting-title">数据备份</div><div className="setting-description mt-0.5">管理本地数据库备份，支持导入、导出和自动备份</div></div></div><div className="mt-4 grid grid-cols-2 gap-2"><button type="button" className="apple-action-button" disabled={importing} onClick={() => void importBackupFromFile()}><Download className="h-4 w-4 text-accent" strokeWidth={2} />导入数据库</button><button type="button" className="apple-action-button" disabled={exporting} onClick={() => void exportBackupToFile()}><Upload className="h-4 w-4 text-success" strokeWidth={2} />导出数据库</button></div><div className="mt-[var(--gap-section)] rounded-[var(--radius-card)] bg-[color-mix(in_srgb,var(--sidebar-bg)_30%,var(--panel-bg))] p-3.5 shadow-[0_0_0_1px_var(--panel-ring)]"><div className="text-[15px] font-semibold tracking-tight">自动备份</div><div className="mt-3 grid gap-3 sm:grid-cols-2"><div><div className="field-label muted mb-1.5">备份间隔</div><AppSelect value={form.auto_backup_interval_hours} options={autoBackupOptions} onChange={(value) => onPatch({ auto_backup_interval_hours: value })} /></div><div><div className="field-label muted mb-1.5">最多保留</div><AppSelect value={form.database_backup_keep_count} options={keepOptions} onChange={(value) => onPatch({ database_backup_keep_count: value })} /></div></div><div className="mt-3 grid grid-cols-2 gap-2"><button type="button" className="apple-action-button" disabled={exporting} onClick={() => void createImmediateBackup()}><Save className="h-4 w-4 text-accent" strokeWidth={2} />立即备份</button><button type="button" className="apple-action-button" onClick={() => { const item = paths.find((path) => path.label === "备份目录"); if (item) onOpenPath(item); else feedback.warning("找不到备份目录"); }}><FolderOpen className="h-4 w-4 text-[var(--warning)]" strokeWidth={2} />备份文件夹</button></div></div><hr className="my-4 border-0 border-t border-[var(--panel-divider)]" /><div className="setting-title mb-2">备份记录</div>{backups.length ? <div className="space-y-2">{backups.map((backup) => <div key={backup.name} className="apple-list-row"><div className="flex min-w-0 items-center gap-2.5"><span className="settings-icon-tile grid h-8 w-8 shrink-0 place-items-center rounded-lg text-accent"><Database className="h-4 w-4" strokeWidth={2} /></span><div className="min-w-0"><div className="mono truncate text-xs font-medium">{backup.name}</div><div className="muted text-[11px]">{formatTimestamp(backup.created_at)} · {formatSize(backup.size_bytes)}</div></div></div><div className="flex shrink-0 gap-1.5"><button type="button" className="apple-icon-button text-zinc-500 hover:text-accent" title="编辑备份名称" onClick={() => openRename(backup)}><Pencil className="h-4 w-4" strokeWidth={2} /></button><button type="button" className="apple-icon-button text-accent" title="恢复数据库" onClick={() => void restoreBackup(backup)}><RefreshCw className="h-4 w-4" strokeWidth={2} /></button><button type="button" className="apple-icon-button text-[var(--danger)]/70" title="删除备份" onClick={() => void deleteBackup(backup)}><TrashIcon /></button></div></div>)}</div> : <div className="setting-description flex items-center gap-2"><Database className="h-4 w-4" />还没有导出过备份。</div>}<AppDialog open={renameTarget !== null} onOpenChange={(open) => { if (!open) setRenameTarget(null); }} title="重命名备份" footer={<><button type="button" className="apple-action-button" onClick={() => setRenameTarget(null)}>取消</button><button type="button" className="apple-action-button app-button--primary" disabled={renaming || !renameText.trim()} onClick={() => void submitRename()}>保存</button></>}><input className="app-input" maxLength={80} placeholder="输入新的备份标题" value={renameText} onChange={(event) => setRenameText(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.nativeEvent.isComposing) void submitRename(); }} /></AppDialog></div>;
}
...
export function SettingsAbout({ paths, onOpenPath, openingPath }: SettingsAboutProps) {
  return <div className="apple-group mt-[var(--gap-section)] p-[var(--gap-card)]"><div className="flex items-center gap-3"><img src="/logo.svg" alt="CGswitch" className="h-12 w-12 shrink-0 dark:invert" /><div><div className="apple-wordmark">CGswitch</div><div className="app-version mt-1.5">版本 {version.trim()}</div></div></div><hr className="my-4 border-0 border-t border-[var(--panel-divider)]" /><h2 className="setting-title">数据与路径</h2><p className="setting-description mt-1.5">常用数据位置，点击文件夹图标即可打开。</p><div className="mt-4 grid gap-2 sm:grid-cols-3">{paths.map((item) => <div key={item.label} className="flex min-w-0 items-center justify-between gap-3 rounded-[var(--radius-control-sm)] border border-[var(--panel-ring)] px-3 py-2.5"><div className="min-w-0"><div className="text-sm font-medium">{item.label}</div><div className="mono muted mt-0.5 truncate text-[11px]" title={item.path}>{item.path}</div></div><button type="button" className="apple-icon-button shrink-0 text-zinc-500 hover:text-accent disabled:opacity-40" disabled={Boolean(openingPath)} title={`打开${item.label}`} onClick={() => onOpenPath(item)}>{openingPath === item.path ? <LoaderCircle className="h-4 w-4 animate-spin" strokeWidth={2} /> : <FolderOpen className="h-4 w-4" strokeWidth={2} />}</button></div>)}</div></div>;
}
