...
export default function ChatGPTAccount({ initialStatus }: { initialStatus: AuthStatus }) {
  ...
  if (login) return <div className="space-y-4"><div className="flex items-start justify-between gap-3"><div className="flex min-w-0 items-start gap-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-accent/10 text-accent"><ShieldCheck className="h-[18px] w-[18px]" strokeWidth={2} /></span><div><div className="setting-title">ChatGPT 设备码登录</div><p className="setting-description mt-0.5">完成 ChatGPT 登录后，认证结果会自动回到这里。</p></div></div><span className="apple-chip chip-warn">等待授权</span></div><div className="rounded-[var(--radius-card)] bg-accent/6 p-4 shadow-[0_0_0_1px_var(--panel-ring)] dark:bg-accent/10"><div className="text-center"><div className="field-label">授权码：请在浏览器中输入此码</div><div className="mt-1 flex items-center justify-center gap-2"><span className="mono whitespace-nowrap text-2xl font-bold tracking-[0.3em]">{login.user_code}</span><button type="button" className="grid h-8 w-8 place-items-center rounded-full text-accent hover:bg-accent/10" title="复制授权码" onClick={() => void navigator.clipboard.writeText(login.user_code).then(() => feedback.success("授权码已复制")).catch(() => feedback.error("复制失败，请手动选择复制"))}><Copy className="h-4 w-4" strokeWidth={2} /></button></div></div><div className="mt-3 border-t border-[var(--panel-border)] pt-3 text-center"><div className="muted text-xs">授权页面</div><button type="button" className="mt-1 flex w-full min-w-0 items-center justify-center gap-1.5 text-sm font-medium text-accent hover:underline" title={login.verification_uri} onClick={() => void api.openUrl(login.verification_uri)}><span className="truncate">{login.verification_uri}</span><ExternalLink className="h-4 w-4 shrink-0" strokeWidth={2} /></button></div><div className="mt-4 flex justify-center"><button type="button" className="apple-action-button" onClick={() => { pollCancelled.current = true; setLogin(null); setBusy(false); }}>取消登录</button></div></div></div>;
  ...
  return <div><div className="rounded-[var(--radius-card)] border border-[var(--panel-border)] bg-black/2 p-4 dark:bg-white/4"><div className="flex items-start gap-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-accent/10 text-accent"><ShieldCheck className="h-[18px] w-[18px]" strokeWidth={2} /></span><div><div className="setting-title">尚未连接 ChatGPT</div><p className="setting-description mt-0.5">登录后可管理多个 ChatGPT 账号。</p></div></div><div className="mt-6"><button type="button" className="apple-action-button app-button--primary" disabled={busy} onClick={() => void startLogin()}><ExternalLink className="h-4 w-4" strokeWidth={2} />使用 ChatGPT 登录</button></div></div>{loadError ? <p className="muted mt-3 text-sm">{loadError}</p> : null}</div>;
}
