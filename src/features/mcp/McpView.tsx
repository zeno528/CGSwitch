import { ArrowsDownUp, CircleDashed, Globe, Plus, TerminalWindow } from "@phosphor-icons/react";
...
export default function McpView() {
  ...
  const transportIcon = (server: McpServerSpec) => { const current = transportOf(server); return current === "http" ? Globe : current === "stdio" ? TerminalWindow : CircleDashed; };
  ...
  return <section className="apple-scroll-page mx-auto w-full max-w-none"><header className="apple-page-bar flex-wrap justify-between gap-4"><div className="flex min-w-0 items-center gap-2.5"><span className="settings-icon-tile grid h-9 w-9 shrink-0 place-items-center rounded-[10px] text-accent"><McpIcon className="h-[22px] w-[22px]" /></span><div className="flex items-center gap-2"><div className="apple-title">MCP 服务器管理</div>{loaded ? <span className="apple-chip" aria-label={`${servers.length} 台服务器`}>{servers.length}</span> : null}</div></div><div className="flex flex-wrap items-center gap-2"><button type="button" className="apple-action-button" disabled={applying} onClick={openSyncDialog}><ArrowsDownUp className="h-4 w-4" weight="bold" />处理配置差异</button><button type="button" className="apple-action-button app-button--primary" onClick={() => setCreatingServer(true)}><Plus className="h-4 w-4" weight="bold" />添加服务器</button></div></header><div className="apple-edit-content">{loadError ? <p className="muted mt-4 text-sm">{loadError}{loaded ? "config.toml 无法解析时，可点「处理配置差异」用数据库中的 MCP 配置恢复。" : ""}</p> : null}<div className="mt-[var(--gap-page)]">{syncPreview && syncPreview.entries.length ? <div className="apple-list-row mb-2"><span className="flex min-w-0 items-center gap-2"><span className="apple-chip chip-warn">MCP 差异</span><span className="muted truncate text-sm">数据库与 config.toml 有 {syncPreview.entries.length} 项 MCP 配置不同</span></span><button type="button" className="apple-inline-btn" onClick={openSyncDialog}>查看并处理</button></div> : null}{loaded && servers.length === 0 ? <div className="apple-group py-14 text-center"><p className="muted">还没有 MCP 服务器。点击“添加服务器”把第一个 MCP 服务写进 config.toml。</p></div> : servers.length ? <div className="space-y-2">{servers.map((server) => { const Icon = transportIcon(server); return <div key={server.name} className="apple-list-row"><button type="button" className="group flex min-w-0 cursor-pointer items-center gap-2.5 text-left" aria-label={`编辑 ${server.name}`} title="点击编辑" onClick={() => setEditingServer(server)}><span className="settings-icon-tile grid h-8 w-8 shrink-0 place-items-center rounded-lg text-accent transition-colors group-hover:bg-[var(--selection-bg)]"><Icon className="h-4 w-4" weight="bold" /></span><div className="min-w-0"><div className="flex items-center gap-2"><span className="min-w-0 truncate font-semibold transition-colors group-hover:text-accent">{server.name}</span><span className="shrink-0 rounded-md bg-black/5 px-1.5 py-px text-[10px] font-medium tracking-wide text-zinc-500 dark:bg-white/10 dark:text-zinc-400">{mcpTransportText(server)}</span></div><div className="mono muted truncate text-[11px]">{metaOf(server)}</div></div></button><div className="flex shrink-0 items-center gap-1.5"><AppSwitch size="sm" checked={server.enabled !== false} disabled={togglingName === server.name} label={`启用 ${server.name}`} onCheckedChange={(value) => void toggleEnabled(server, value)} /><button type="button" className="apple-icon-button text-[var(--danger)]/70 hover:bg-[var(--danger)]/10 hover:text-[var(--danger)]" title="删除" aria-label={`删除 ${server.name}`} onClick={() => void removeServer(server)}><TrashIcon /></button></div></div>; })}</div> : null}</div></div><McpSyncDialog open={syncOpen} preview={syncPreview} previewError={previewError} busy={applying} onClose={() => setSyncOpen(false)} onApply={(direction) => void onApply(direction)} /></section>;
}
