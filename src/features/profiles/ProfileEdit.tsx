...
export default function ProfileEdit({ profile, create = false, onBack, onChanged }: ProfileEditProps) {
  ...
  const renderAccountLabel = (option: { label: string; value: string }) => {
    ...
    const Icon = desktop ? Monitor : Key;
    return <span className="inline-flex min-w-0 items-center gap-2"><Icon className="h-3.5 w-3.5 shrink-0 text-accent" weight="bold" aria-hidden="true" /><span className="shrink-0 text-xs font-medium text-[var(--text-secondary)]">{desktop ? "桌面端认证" : "OAuth 认证"}</span><span className="text-[var(--text-secondary)]">·</span><span className="truncate">{option.label}</span></span>;
  };
  ...
  return (
    <section className="apple-edit-page mx-auto flex w-full max-w-none flex-col" onKeyDown={(event) => { if (event.ctrlKey && event.key === "Enter") void save(); }}>
      <div className="apple-page-bar apple-page-bar--roomy apple-edit-toolbar apple-edit-toolbar--header">
        <button type="button" className="apple-page-header apple-back-button" aria-label="返回" onClick={onBack}><ArrowLeft className="h-4 w-4 shrink-0 text-accent" weight="bold" aria-hidden="true" /><span className="apple-title">{create ? "新建供应商" : "编辑供应商"}</span></button>
      </div>
