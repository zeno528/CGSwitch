  ...
  return (
    <section className="apple-scroll-page mx-auto w-full max-w-none">
      <header className="apple-page-bar flex-wrap justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2"><button type="button" className="apple-action-button apple-action-button--quaternary" disabled={busy || (restartStage !== "idle" && restartStage !== "success" && restartStage !== "error")} title="重启 Codex" onClick={() => void restart(false)}><ArrowClockwise className="h-4 w-4" weight="bold" />重启 Codex</button><button type="button" className="apple-icon-button text-accent hover:bg-[var(--sidebar-bg)]" disabled={busy} title="捕获当前配置" aria-label="捕获当前配置" onClick={openCapture}><Camera className="h-4 w-4" weight="bold" /></button><button type="button" className="apple-action-button app-button--primary" disabled={busy} onClick={() => setCreatingProfile(true)}><Plus className="h-4 w-4" weight="bold" />添加供应商</button></div>
      </header>
